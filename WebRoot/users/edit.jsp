...
<script type="text/javascript">
	...
	function add() {
		...
		if(!$(":text[name=sname]").val()) {
			$("#cnameError").text("学生名称不能为空");
			bool = false;
		}
		if(!$("#male").attr("checked") && !$("#female").attr("checked")) {
			$("#genderError").text("学生性别不能为空");
			bool = false;
		}
		if(!$(":text[name=tellphone]").val()) {
...
<table border="0" align="center" width="40%" style="margin-left: 100px;">
	...
	<tr>
		<td>学生性别</td>
		<td>
			<input type="radio" name="gender" value="男" id="male" <c:if test="${student.gender eq '男' }">checked='checked'</c:if> />
			<label for="male">男</label>
			<input type="radio" name="gender" value="女" id="female" <c:if test="${student.gender eq '女' }">checked='checked'</c:if> />
	...
	<tr>
		<td>学生生日</td>
		<td>
			<input type="text" name="birthday"  value="${student.birthday }" id="birthday" readonly="readonly"/>
		</td>
		<td>
	...
	<tr>
		<td>手机</td>
		<td>
			<input type="text" name="tellphone" value="${student.tellphone }" />
		</td>
		<td>
	...
	<tr>
		<td>&nbsp;</td>
		<td>
			<input type="button" value="编辑学生" onclick="add()"/>
